...
export const useViewOptions = ({ folders, activeFolderIds }: UseViewOptionsProps) => {
  ...
  // Memoize Fuse.js instance to avoid recreation on every render/keystroke
  ...
  const availableTags = useMemo(() => {
    ...
    currentItems.forEach((item) => {
      item.aiTags?.forEach((tag) => tags.add(tag));
      item.manualTags?.forEach((tag) => tags.add(tag));
    });
  ...
  const processedItems = useMemo(() => {
    ...
    // Fuzzy Search using memoized index
    if (searchTerm && fuseIndex) {
      result = fuseIndex.search(searchTerm).map((res) => res.item);
    }

    // Filters
    if (selectedTag)
      result = result.filter((item) => item.aiTags?.includes(selectedTag));
    if (activeColorFilter)
      result = result.filter((item) => item.colorTag === activeColorFilter);
