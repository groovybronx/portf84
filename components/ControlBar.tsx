...
export const ControlBar: React.FC<ControlBarProps> = ({ 
  ...
  return (
    <motion.div 
      ...
      className="fixed bottom-8 left-1/2 transform -translate-x-1/2 z-50 max-w-[90vw]"
    >
      <div className="bg-surface/80 backdrop-blur-xl border border-white/10 rounded-full shadow-2xl shadow-black/50 overflow-hidden">
            ...
            {selectionMode ? (
                ...
                >
                    ...
                    <button 
                        ...
                        className="flex items-center gap-2 px-4 py-2 rounded-full hover:bg-white/10 text-white transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                ...
                >
                    <div className="pr-4 border-r border-white/10 flex items-center gap-3">
                        <button 
                            onClick={onOpenFolders}
                            className="p-3 rounded-full hover:bg-white/10 transition-colors text-blue-400 hover:text-white flex items-center gap-2"
                    ...
                    {modes.map((mode) => (
                        ...
                        className={`relative px-3 sm:px-4 py-2 rounded-full flex items-center gap-2 transition-all duration-300 ${
                            currentMode === mode.id ? 'text-white' : 'text-gray-500 hover:text-gray-300'
                        }`}
                        >
                        {currentMode === mode.id && (
                            <motion.div
                        ...
                        <span className="relative z-10 text-sm font-medium hidden sm:block">{mode.label}</span>
                        </button>
                    ))}
                    </div>
                </motion.div>
