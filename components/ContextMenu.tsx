...
import { Sparkles, Trash2, Eye, Palette, Copy, X } from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      <button
        ...
        className="w-full text-left px-4 py-2 text-sm text-gray-200 hover:bg-white/10 hover:text-white flex items-center gap-3 transition-colors"
      >
        <Eye size={16} className="text-blue-400" /> Open Fullscreen
      </button>

      <button
        onClick={() => { onAnalyze(item); onClose(); }}
        className="w-full text-left px-4 py-2 text-sm text-gray-200 hover:bg-white/10 hover:text-white flex items-center gap-3 transition-colors"
      >
      ...
      <div className="px-4 py-2">
        <p className="text-xs text-gray-500 uppercase mb-2 flex items-center gap-2"><Palette size={12}/> Color Tag</p>
        <div className="flex justify-between">
          {Object.entries(COLOR_PALETTE).map(([key, hex]) => (
            <button
              key={key}
              onClick={() => { onColorTag(item, hex); onClose(); }}
              className={`w-5 h-5 rounded-full border border-transparent hover:scale-110 hover:border-white transition-all ${item.colorTag === hex ? 'ring-2 ring-white/50' : ''}`}
