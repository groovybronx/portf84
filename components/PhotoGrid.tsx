...
export const PhotoGrid: React.FC<PhotoGridProps> = ({ 
  ...
  return (
    <div className="p-4 sm:p-8 pb-10 pt-36 w-full min-h-screen" ref={containerRef}>
                ...
                {colItems.map((item) => {
                    ...
                    return (
                        <motion.div
                            ...
                            animate={{ opacity: 1, y: 0, scale: selectionMode && isSelected ? 0.95 : 1 }}
                        ...
                        >
                            ...
                            {!selectionMode && (
                                <div className={`absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent transition-opacity duration-300 flex flex-col justify-end p-4 ${isFocused ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
                                    <p className="text-white text-sm font-medium truncate">{item.name}</p>
                                    <p className="text-white/60 text-xs uppercase tracking-wider">{(item.size / 1024 / 1024).toFixed(2)} MB</p>
                                </div>
                            )}
                            ...
                            {selectionMode && (
                                <div className="absolute inset-0 bg-black/20 flex items-start justify-end p-3">
                                    {isSelected ? (
                                        <CheckCircle2 className="text-blue-500 drop-shadow-lg bg-white rounded-full" size={24} fill="white" />
                                    ) : (
      ...
      <div ref={observerTarget} className="w-full h-20 flex items-center justify-center text-white/20">
         {visibleCount < items.length && <div className="animate-pulse">Loading more...</div>}
      </div>
    </div>
  );
};