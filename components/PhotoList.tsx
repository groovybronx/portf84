...
export const PhotoList: React.FC<PhotoListProps> = ({ 
  ...
  return (
    <div className="w-full max-w-5xl mx-auto pt-36 pb-10 px-4 sm:px-8">
            ...
            return (
                ...
                >
                    ...
                    {selectionMode && (
                        <div className="flex-shrink-0">
                            {isSelected ? (
                                ...
                                <Circle className="text-gray-500 group-hover:text-white" size={24} />
                            )}
                        </div>
                    )}
                    ...
                    <div className="flex-1 min-w-0">
                        <h3 className={`text-lg font-medium transition-colors truncate ${isSelected ? 'text-blue-400' : 'text-white group-hover:text-blue-400'}`}>
                            {item.name}
                        </h3>
                        <div className="flex items-center gap-4 mt-2 text-sm text-gray-500 font-mono">
                            <span className="flex items-center gap-1"><FileImage size={14} /> {item.type.split('/')[1].toUpperCase()}</span>
                            <span>•</span>
                    ...
                    <div className="hidden sm:flex flex-col items-end gap-2">
                        {item.aiTags && (
                            <div className="flex gap-1">
                                {item.aiTags.slice(0, 2).map(tag => (
                                    <span key={tag} className="text-xs bg-white/5 px-2 py-1 rounded-md text-gray-400 border border-white/5 flex items-center gap-1">
                                        <Tag size={10} /> {tag}
                                    </span>
                                ))}
                            </div>
                        ...
                        {!selectionMode && (
                            <button className="p-2 rounded-full hover:bg-white/10 text-gray-600 hover:text-white transition-colors">
                                <Info size={18} />
                            </button>
                        )}
                    </div>
