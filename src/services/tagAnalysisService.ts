 ...
 */
const levenshteinDistance = (a: string, b: string, threshold: number = Infinity): number => {
...
const areTokensSimilar = (a: Set<string>, b: Set<string>, threshold: number = 0.8): boolean => {
...
export const analyzeTagRedundancy = async (
  ...
  logger.debug('app', '[TagAnalysis] Cache MISS - Running analysis');
  const ignoredMatches = await getIgnoredMatches();
  const ignoredSet = new Set(ignoredMatches.map((pair) => pair.sort().join('|')));

  logger.debug('app', '[TagAnalysis] Analyzing ${tags.length} tags for redundancy...');
  ...
  logger.debug('app', '[TagAnalysis] Found ${groups.length} groups with duplicates');
