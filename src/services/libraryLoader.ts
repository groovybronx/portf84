...
export const libraryLoader = {
  ...
  loadAndMerge: async (
    ...
    // 1. Scan Disk
    logger.debug('app', '[LibraryLoader] Scanning: ${basePath}');
    const { folders: diskFolderMap, looseFiles } = await scanDirectory(basePath);
    logger.debug(
      'app',
    ...
    const allFilePaths = [
      ...Array.from(diskFolderMap.values()).flatMap((items) => items.map((i) => i.path || i.name)),
      ...looseFiles.map((i) => i.path || i.name),
    ];
    logger.debug('app', `[LibraryLoader] Fetching metadata for ${allFilePaths.length} items...`);
    const metaMap = await storageService.getMetadataBatch(allFilePaths, collectionId);
    logger.debug('app', `[LibraryLoader] Metadata fetched: ${metaMap.size} entries found in DB.`);
    ...
    const sourceFoldersForCollection = await storageService.getCollectionFolders(collectionId);
    const sourceFolderForPath = sourceFoldersForCollection.find((sf) => sf.path === basePath);
    const shadowFolderForPath = sourceFolderForPath
      ? storedVirtualFolders.find((vf) => vf.sourceFolderId === sourceFolderForPath.id)
      : null;
    ...
    if (shadowFolderForPath) {
      ...
      logger.debug('app', `[LibraryLoader] No shadow folder found for path: ${basePath}`);
    }
    ...
    const processItem = (
      ...
      if (meta) {
        const vfId = meta.virtualFolderId ?? meta.folderId;
        const hydrated: PortfolioItem = {
          ...item,
          collectionId,
          ...(meta.aiDescription ? { aiDescription: meta.aiDescription } : {}),
          aiTags: meta.aiTags,
          aiTagsDetailed: meta.aiTagsDetailed,
          ...(meta.colorTag ? { colorTag: meta.colorTag } : {}),
          manualTags: meta.manualTags,
          ...(typeof vfId === 'string' ? { virtualFolderId: vfId } : {}),
          ...(typeof vfId === 'string' ? { folderId: vfId } : {}),
        };

        // If the item belongs to a known virtual folder, return that ID
        if (hydrated.virtualFolderId && virtualFolderIds.has(hydrated.virtualFolderId)) {
      ...
      const itemWithCollection: PortfolioItem = { ...item, collectionId };
      if (shadowFolderForPath) {
        return {
    ...
    logger.debug('app', `[LibraryLoader] Returning ${physicalFolders.length} physical folders`);
