 ...
 */
export const getVirtualFolders = async (collectionId?: string): Promise<ParsedVirtualFolder[]> => {
  const db = await getDB();
...
export const createShadowFolder = async (
  ...
  const db = await getDB();
  const id = `shadow-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`;
...
export const getShadowFoldersWithSources = async (
  ...
  const results = shadowFolders
    ...
    .map((shadow) => {
      const source = sourceFolders.find((sf) => sf.id === shadow.sourceFolderId);
...
export const addFolderToCollection = async (
  ...
  logger.debug('storage', '[Storage] addFolderToCollection called with:', {
    collectionId,
    path,
  });
  const db = await getDB();
  ...
  logger.debug('storage', '[Storage] Inserting into collection_folders:', {
    id,
    collectionId,
    path,
    name,
  ...
  try {
    ...
    logger.debug('storage', `[Storage] ✅ Folder successfully added to collection: ${path}`);
    ...
    logger.debug('storage', '[Storage] Verification - Inserted row:', verify);
    ...
    logger.debug('storage', `[Storage] ✅ Shadow folder created automatically: ${shadowId}`);

    return shadowId;
  } catch (error) {
    logger.debug('storage', '[Storage] ❌ Error adding folder to collection:', {
...
export const removeFolderFromCollection = async (
  ...
  const db = await getDB();
  await db.execute('DELETE FROM collection_folders WHERE collectionId = ? AND path = ?', [
    collectionId,
    path,
  ]);
  logger.debug('storage', `[Storage] Folder removed from collection: ${path}`);
};

/**
 * Get all source folders for a collection
 */
export const getCollectionFolders = async (collectionId: string): Promise<DBCollectionFolder[]> => {
  const db = await getDB();
  const result = await db.select<DBCollectionFolder[]>(
    'SELECT * FROM collection_folders WHERE collectionId = ? ORDER BY addedAt DESC',
    [collectionId]
  );
  logger.debug('storage', `[Storage] getCollectionFolders(${collectionId}):`, {
    folders: result.length,
  });
  return result;
};
...
export const debugListAllCollectionFolders = async (): Promise<DBCollectionFolder[]> => {
  ...
  logger.debug('storage', '[Storage] DEBUG - All collection_folders in DB:', {
...
export const groupItemsByColorTag = async (
  ...
  for (const itemId of itemIds) {
    await db.execute('UPDATE metadata SET virtualFolderId = ? WHERE id = ?', [folderId, itemId]);
    count++;
  }

  logger.debug('app', '[Storage] ✅ Created folder "${folderName}" with ${count} items');
