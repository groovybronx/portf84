...
export const saveMetadata = async (
  ...
  try {
    ...
    logger.debug('app', '[Storage] Metadata saved for ${relativePath}');
    ...
    try {
      ...
      logger.debug('app', '[Storage] Tags synced to relational DB for ${relativePath}');
    } catch (err) {
      logger.error('app', '[Storage] Failed to sync tags for ${relativePath}:', err);
      // Be careful not to throw here, we don't want to break the main metadata save
    }
  } catch (e) {
    logger.error('app', '[Storage] Failed to save metadata:', e);
  }
};
...
export const getMetadataBatch = async (
  ...
  if (keys.length === 0) {
    ...
    if (collectionId) {
      results = await db.select<DBMetadata[]>('SELECT * FROM metadata WHERE collectionId = ?', [
        collectionId,
      ]);
      logger.debug(
        'app',
    ...
    for (const key of keys) {
      ...
      if (collectionId) {
        ...
        row = await db.select<DBMetadata[]>('SELECT * FROM metadata WHERE id = ?', [key]);
      }
  ...
  results.forEach((meta) => {
    ...
    resultMap.set(meta.id, {
      ...
      folderId: meta.virtualFolderId ?? null,
      aiDescription: meta.aiDescription,
      aiTags: relationalTags?.ai.length ? relationalTags.ai : JSON.parse(meta.aiTags || '[]'),
      aiTagsDetailed: relationalTags?.detailed.length
        ? relationalTags.detailed
        : JSON.parse(meta.aiTagsDetailed || '[]'),
      colorTag: meta.colorTag,
      manualTags: relationalTags?.manual.length
        ? relationalTags.manual
        : JSON.parse(meta.manualTags || '[]'),
...
export const hideImage = async (imageId: string): Promise<void> => {
  ...
  logger.debug('app', '[Storage] Image hidden: ${imageId}');
};
...
export const unhideImage = async (imageId: string): Promise<void> => {
  ...
  logger.debug('app', '[Storage] Image restored: ${imageId}');
};
