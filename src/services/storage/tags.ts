...
export const getOrCreateTag = async (
  ...
  logger.debug('app', '[Storage] Tag created: ${name} (${type})');
 ...
 */
export const addTagToItem = async (itemId: string, tagId: string): Promise<void> => {
  const db = await getDB();
  await db.execute('INSERT OR IGNORE INTO item_tags (itemId, tagId, addedAt) VALUES (?, ?, ?)', [
    itemId,
    tagId,
    Date.now(),
  ]);
 ...
 */
export const removeTagFromItem = async (itemId: string, tagId: string): Promise<void> => {
  const db = await getDB();
  await db.execute('DELETE FROM item_tags WHERE itemId = ? AND tagId = ?', [itemId, tagId]);
};
...
export const getTagsForItem = async (itemId: string): Promise<ParsedTag[]> => {
  ...
  return rows.map((t) => ({
    ...
    ...(t.parentId !== null ? { parentId: t.parentId } : {}),
  }));
};
...
export const getAllTags = async (type?: TagType): Promise<ParsedTag[]> => {
  ...
  if (type) {
    rows = await db.select<DBTag[]>('SELECT * FROM tags WHERE type = ? ORDER BY name', [type]);
  } else {
    rows = await db.select<DBTag[]>('SELECT * FROM tags ORDER BY type, name');
  }

  logger.debug('storage', `[Storage] getAllTags(${type || 'ALL'}) returned ${rows.length} rows`);
...
export const deleteTag = async (tagId: string): Promise<void> => {
  ...
  logger.debug('app', '[Storage] Tag deleted: ${tagId}');
  invalidateAnalysisCache();
};
...
export const mergeTags = async (
  ...
  try {
    ...
    logger.debug('app', '[Storage] Merged ${sourceTagIds.length} tags into ${targetTagId}');
    invalidateAnalysisCache();
  } catch (error) {
    logger.error('app', 'Failed to merge tags:', error);
    throw error;
  }
};
...
export const undoMerge = async (mergeId: string): Promise<void> => {
  ...
  // 1. Get merge info
  const mergeRows = await db.select<
    Array<{
      targetTagId: string;
      sourceTagId: string;
  ...
  >('SELECT * FROM tag_merges WHERE id = ?', [mergeId]);
  ...
  try {
    ...
    if (merge.itemIdsJson) {
      try {
        ...
        logger.warn('app', 'Failed to parse merge snapshot:', e);
      }
    }
    ...
    await db.execute(
      ...
      [
        sourceTagId,
        merge.sourceTagName,
        merge.sourceTagName.toLowerCase().trim(),
        'manual',
        Date.now(),
      ]
    );
    ...
    if (snapshot) {
      ...
      logger.warn(
        'app',
        'Undoing legacy merge without snapshot. Linkages cannot be fully restored.'
      );
    }
    ...
    logger.debug('app', '[Storage] Undo merge complete: ${merge.sourceTagName} restored.');
    invalidateAnalysisCache();
  } catch (error) {
    logger.error('app', 'Failed to undo merge:', error);
    throw error;
  }
};
...
export const ignoreTagMatch = async (tagId1: string, tagId2: string): Promise<void> => {
  ...
  logger.debug('app', '[Storage] Tag match ignored: ${tagId1} <-> ${tagId2}');
};
...
export const syncAllTagsFromMetadata = async (): Promise<number> => {
  const db = await getDB();
  logger.debug('app', '[Storage] Starting full tag resync...');
  ...
  for (const row of rows) {
    ...
    try {
      aiTags = JSON.parse(row.aiTags || '[]');
    } catch {}
    try {
      manualTags = JSON.parse(row.manualTags || '[]');
    } catch {}
    ...
    for (const tagName of aiTags) {
      ...
      try {
        await addTagToItem(row.id, tagId);
      } catch {}
    }
  ...
  logger.debug('app', '[Storage] Resync complete. Processed ${count} items.');
  return count;
};
 ...
 */
export const createTagAlias = async (aliasName: string, targetTagId: string): Promise<void> => {
  const db = await getDB();
  ...
  logger.debug('app', '[Storage] Alias created: ${aliasName} → ${targetTagId}');
};
...
export const getTagByAlias = async (aliasName: string): Promise<ParsedTag | null> => {
  ...
  return {
    ...
    ...(tag.parentId !== null ? { parentId: tag.parentId } : {}),
  };
};
...
export const deleteTagAlias = async (aliasName: string): Promise<void> => {
  const db = await getDB();
  await db.execute('DELETE FROM tag_aliases WHERE aliasName = ?', [aliasName.toLowerCase().trim()]);
  logger.debug('app', '[Storage] Alias deleted: ${aliasName}');
};
 ...
 */
export const setTagParent = async (tagId: string, parentId: string | null): Promise<void> => {
  const db = await getDB();
  ...
  await db.execute('UPDATE tags SET parentId = ? WHERE id = ?', [parentId, tagId]);

  logger.debug('app', '[Storage] Tag parent updated: ${tagId} -> ${parentId}');
};
...
export const renameTag = async (tagId: string, newName: string): Promise<void> => {
  ...
  await db.execute('UPDATE tags SET name = ?, normalizedName = ? WHERE id = ?', [
    newName,
    normalizedName,
    tagId,
  ]);

  logger.debug('app', '[Storage] Tag renamed: ${tagId} -> ${newName}');
  invalidateAnalysisCache();
};
...
export const bulkDeleteTags = async (tagIds: string[]): Promise<void> => {
  ...
  logger.debug('app', '[Storage] Bulk deleted ${tagIds.length} tags');
  invalidateAnalysisCache();
};
