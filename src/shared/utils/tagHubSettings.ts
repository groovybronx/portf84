...
export const loadTagHubSettings = (): TagHubSettings => {
  try {
    ...
    if (!parsed || typeof parsed !== 'object') {
      logger.warn('app', '[TagHubSettings] Invalid stored data, using defaults');
      return DEFAULT_TAGHUB_SETTINGS;
    }
    ...
    if (
      !isValidEnum(parsed.viewMode, validViewModes) ||
      ...
      logger.warn('app', '[TagHubSettings] Invalid property values, using defaults');
      return DEFAULT_TAGHUB_SETTINGS;
    }

    return parsed as TagHubSettings;
  } catch (error) {
    logger.error('app', '[TagHubSettings] Failed to load:', error);
    return DEFAULT_TAGHUB_SETTINGS;
  }
};
...
export const saveTagHubSettings = (settings: TagHubSettings): void => {
  try {
    ...
    logger.debug('app', '[TagHubSettings] Saved successfully');
  } catch (error) {
    logger.error('app', '[TagHubSettings] Failed to save:', error);
    ...
    if (error instanceof Error && error.name === 'QuotaExceededError') {
      logger.error('app', '[TagHubSettings] Storage quota exceeded. Settings may not persist.');
    }
  }
};
...
const migrateTagHubSettings = (old: Partial<TagHubSettings>): TagHubSettings => {
  logger.debug('app', '[TagHubSettings] Migrating from version', {
    from: old.version,
    to: CURRENT_VERSION,
  });
