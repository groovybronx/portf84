...
import { Sparkles, Trash2, Eye, Palette, X, Tag, FolderInput } from 'lucide-react';
  ...
  return (
    ...
    >
      <div className="px-4 py-2 border-b border-glass-border/10 mb-1 flex items-center justify-between">
        <p className="text-xs font-mono text-gray-500 truncate max-w-[150px]" title={item.name}>
          {item.name}
        </p>
        <Button onClick={onClose} variant="close" size="icon-sm" aria-label={t('closeMenu')}>
          <X size={14} />
        </Button>
      </div>
      ...
      >
        ...
        <Trash2 size={16} className="relative z-10 transition-transform group-hover:scale-110" />
