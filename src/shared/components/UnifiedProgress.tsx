...
const TaskItem: React.FC<{ task: ProgressTask; onRemove: () => void }> = ({ task, onRemove }) => {
  ...
  return (
    <GlassCard
      ...
      className="pointer-events-auto group relative overflow-hidden rounded-2xl! shadow-2xl"
    >
      <div className="flex items-start gap-3">
        <div className="mt-0.5">
          {task.status === 'active' && <Loader2 className="w-5 h-5 text-blue-500 animate-spin" />}
        ...
        <div className="flex-1 min-w-0">
          <div className="flex justify-between items-start">
            <h4 className="text-sm font-semibold text-white truncate pr-6">{task.label}</h4>
            <Button
              onClick={onRemove}
              variant="close"
              size="icon-sm"
          ...
          {task.message && <p className="text-xs text-white/50 mt-0.5 truncate">{task.message}</p>}

          <div className="mt-3 space-y-1.5">
            <div className="h-1 w-full bg-glass-bg-accent rounded-full overflow-hidden">
              <motion.div
                className={`h-full ${
                  isError ? 'bg-red-500' : isCompleted ? 'bg-emerald-500' : 'bg-blue-500'
                }`}
                initial={{ width: 0 }}
