...
import { RotateCcw, Tag, Sparkles, HardDrive, FolderHeart } from 'lucide-react';
    ...
    >
      ...
      {locationInfo && (
        <Flex align="center" gap="sm">
          ...
          <span className="text-xs text-gray-400 truncate">{locationInfo.name}</span>
        </Flex>
      )}
      ...
      <Stack spacing="sm" className="flex-1 min-h-0">
        ...
        <Flex wrap="wrap" gap="sm">
          {(item.manualTags || []).length === 0 && (item.aiTags || []).length === 0 && (
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-3 text-[11px] font-mono shrink-0">
        <Stack spacing="xs">
          <span className="text-[9px] uppercase tracking-widest text-gray-500">{t('type')}</span>
          <span className="text-gray-300">
            {item.type.split('/')[1]?.toUpperCase() || 'UNKNOWN'}
          </span>
        </Stack>
        <Stack spacing="xs">
          <span className="text-[9px] uppercase tracking-widest text-gray-500">{t('size')}</span>
          <span className="text-gray-300">{(item.size / 1024 / 1024).toFixed(2)} MB</span>
        </Stack>
        {item.width && (
