...
export const PhotoCarousel: React.FC<PhotoCarouselProps> = ({ onSelect, onFocusedItem }) => {
   ...
   */
  useEffect(() => {
    const imagesToUnload = items.filter((_, idx) => Math.abs(idx - currentIndex) > 2).slice(0, 10); // Limiter pour éviter trop d'opérations
              ...
              <div className="relative max-w-3xl max-h-full flex items-center justify-center">
                ...
                {isCurrent && (
                  <div className="absolute bottom-0 left-0 right-0 bg-linear-to-t from-black/80 	to-transparent p-6 rounded-b-lg">
                    ...
                    {item.aiDescription && (
                      <p className="text-gray-300 text-sm mb-2">{item.aiDescription}</p>
                    )}
