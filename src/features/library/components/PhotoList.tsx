    ...
    <div className="h-full overflow-y-auto px-8 py-6">
      <div className="max-w-6xl mx-auto space-y-4">
        {items.map((item, index) => {
          ...
          const fileExtension = item.type.split('/')[1]?.toUpperCase() || 'FILE';
          ...
          return (
            ...
            >
              ...
              <GlassCard
                variant="accent"
                padding="none"
                className={`shrink-0 w-32 h-32 overflow-hidden ${
                  !loadedItems.has(item.id) ? 'animate-pulse' : ''
                }`}
              >
                <img
                  src={item.url}
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start gap-2 mb-2">
                  <h3 className="text-lg font-semibold text-white truncate flex-1">{item.name}</h3>
                ...
                {item.aiDescription && (
                  <p className="text-sm text-gray-400 mb-2 line-clamp-2">{item.aiDescription}</p>
                )}
