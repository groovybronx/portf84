...
export const TagInput: React.FC<TagInputProps> = ({
  ...
  const suggestions = React.useMemo(() => {
    ...
    return availableTags
      .filter((tag) => tag.toLowerCase().includes(currentPart) && !existingTags.has(tag))
      .slice(0, 5);
  ...
  return (
    <Stack spacing="sm">
      <Flex align="center" gap="sm">
        <span className="text-sm font-medium text-white">➕ {t('addTags')}</span>
      </Flex>
      <div className="relative">
        ...
        {/* Helper text */}
        <p className="text-xs text-gray-500 mt-1.5 px-1">{t('tagInputHint')}</p>
      </div>
    </Stack>
  );
};
