  ...
  try {
    ...
    if (item.file) {
      ...
      try {
        ...
        logger.error('network', 'Failed to fetch image from URL:', err);
    ...
    logger.error('app', 'Gemini Analysis Error:', error);
    ...
    throw new GeminiError(
      error.message || i18next.t('errors:unknownAnalysisError'),
      'UNKNOWN_ERROR'
    );
  }
};
...
export const analyzeImageStream = async (
  ...
  try {
    ...
    logger.error('app', 'Gemini Stream Error:', error);
    if (error instanceof GeminiError) throw error;
